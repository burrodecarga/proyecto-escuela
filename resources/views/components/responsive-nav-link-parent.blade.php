...
<div x-data="{ open: false }" @click.away="open = false" @close.stop="open = false" @click="open = ! open"
    ...
    <div>
        ...
        <div class="relative inline-block ml-1 top-1"></div> <svg class="w-4 h-4 fill-current"
...
<div class="mt-3 border border-gray-300 children" x-show="open" x-transition:enter="transition ease-out duration-200"
    x-transition:enter-start="transform opacity-0 scale-95" x-transition:enter-end="transform opacity-100 scale-100"
    x-transition:leave="transition ease-in duration-75" x-transition:leave-start="transform opacity-100 scale-100"
    x-transition:leave-end="transform opacity-0 scale-95"> {{ $children }} </div>
</div>
